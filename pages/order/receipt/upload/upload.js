...
const { uploadImgs } = require('../../../../js/upload-img/uploadImgs.js')
const app = getApp();
		...
		app.query('/api/pay/payByBank', queryData, 'POST').then(res => {
			...
			wx.showModal({
				...
				success: (res) => {
					if(res.confirm) {
						wx.navigateTo({
							url: '../../../customer/myorders/orders/orders',
						})
					}
				}
			})
			...
			if (err.statusCode === 200 && err.data.status === 500) {
				wx.showModal({
					...
					success: () => {
						wx.navigateTo({
							url: `../../../customer/myorders/detail/detail?order_id=${this.data.payment.order_id}`,
						})
					}
				})
			} else {
