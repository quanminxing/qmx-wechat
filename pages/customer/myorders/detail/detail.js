...
const { each, formatTime } = require('../../../../js/util.js')
...
Page({
	...
	data: {
		...
		sideNav: {
			...
			nav: [
				{
					icon: imagesUrl + '/home_press-black.png',
					label: '首页',
					classname: 'nav-item1',
					url: '../../../index/index'
				},
				{
					icon: imagesUrl + '/dsp_press-black.png',
					label: '样片库',
					classname: 'nav-item2',
					url: '../../../product/store/videos/videos'
				}, {
					icon: imagesUrl + '/wd_press-black.png',
					label: '我的',
					classname: 'nav-item3',
					url: '../../../customer/my/my'
				}
		...
		send: [
			{
				icon: imagesUrl + '/address.png',
				key: 'address',
				value: '浙江省杭州市余杭区仓前街道海智中心4幢7楼宜拍短视频'
			},
			{
				icon: imagesUrl + '/account.png',
				key: 'worker_name',
				value: ''
			},
			{
				icon: imagesUrl + '/phone.png',
				key: 'worker_phone',
				value: ''
			}
		],
	...
	modifyReserve() {
		...
		wx.navigateTo({
			url: '../../../order/revise-reservation/revise-reservation'
		})
	},
	...
	toPay(e) {
		const payment = {
			...
			voucher: () => {  // 对公付款
				...
				console.log(payment)
				wx.navigateTo({
					url: `../../../order/receipt/upload/upload?order_id=${payment.order_id}&type=${payment.pay_type}&price=${payment.pay_price}`
				})
			}
		}
